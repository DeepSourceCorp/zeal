...
export default {
  ...
  render(h) {
    const mWidth = (this.maxWidth && `max-w-${this.maxWidth}`) || '',
      headerWrapperStyle = `${
        this.isScrolling || (!this.isUserOnTop && 'lg:backdrop-blur lg:bg-opacity-25')
      } ${
        this.isUserOnTop && 'lg:border-0'
      } fixed left-0 top-0 flex z-100 justify-center w-full max-w-full bg-ink-400 border-b border-ink-200 min-h-16`,
